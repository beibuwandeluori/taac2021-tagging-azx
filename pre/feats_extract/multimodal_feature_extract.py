...
from pre.feats_extract.imgfeat_extractor.VIT_H14 import VITH14ACExtractor
from pre.feats_extract.imgfeat_extractor.VIT_L16 import VITL16ACExtractor
...
class MultiModalFeatureExtract(object):
    ...
    def __init__(self, batch_size = 1, 
        ...
        if extract_video:
            ...
            elif imgfeat_extractor == 'VIT_B16':
                self.extractor = VITB16ACExtractor()
                self.extractor.eval()
            elif imgfeat_extractor == 'VIT_L16':
                self.extractor = VITL16ACExtractor()
                self.extractor.eval()
            elif imgfeat_extractor == 'VIT_H14':
                self.extractor = VITH14ACExtractor()
                self.extractor.eval()
            else:
        ...
        if (frame_npy_path is None or os.path.exists(frame_npy_path)) and save==True:
            ...
            if self.batch_size == 2:
                ...
                for rgb in self.frame_iterator(test_file, every_ms=1000.0/FRAMES_PER_SECOND):
                    ...
                    features = self.extractor.extract_rgb_frame_features(
                        transform_train(img).view(1, 3, 224, 224)).squeeze().detach().type(torch.FloatTensor).numpy()
...
#                     .view(1,3,224,224)
                ...
                feat_dict['video'] = self.extractor.extract_rgb_frame_features(real_rgb_list).detach().type(torch.FloatTensor).numpy()
            end_time = time.time()
