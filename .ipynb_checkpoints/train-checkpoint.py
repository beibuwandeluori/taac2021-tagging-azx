...
from src.data.dataset import TencentDataset, create_val_transforms, create_train_transforms
import yaml
...
def eval_model(model, epoch, eval_loader, is_save=True):
    ...
    with torch.no_grad():
        for i, (video_feature, audio_feature, text, image, label) in enumerate(eval_process):
            if i > 0:
...
#     pgd = PGD(model)
    ...
    for i, (video_feature, audio_feature, text, image, label) in enumerate(training_process):
        if i > 0:
        ...
        y_pred_adv = model(video_feature, audio_feature, text, image)
...
if __name__ == '__main__':
    ...
    if is_train:
        ...
        xdl = TencentDataset(root_path=root_path, gt_txt=gt_txt, class_txt=class_txt, vocab=vocab, phase='train',
        ...
        xdl_eval = TencentDataset(root_path=root_path, gt_txt=gt_txt, class_txt=class_txt, vocab=vocab, phase='val',
