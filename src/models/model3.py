...
class Model(nn.Module):
    def __init__(self, config, video_embed_dim=1280, video_embed_dim2=1024, audio_embed_dim=128, hidden_dim=512, modal_dim=1024,
        ...
        if self.with_video_head:
            ...
            #             video_dim = 16384
            video_dim2 = self.video_embed_dim2
        # 创建audio模型
        audio_dim = 0
        ...
        out_size = video_dim + video_dim2 + audio_dim + text_out_dim + image_out_dim
        if self.classifier_type == 'ClassificationHead':
            self.classifier = ClassificationHead(out_size=out_size, hidden_dim=self.hidden_dim, dropout_prob=0.5,
    ...
    def forward(self, video_feature,video_feature2, audio_feature, text, image, image_only=False):
        """
