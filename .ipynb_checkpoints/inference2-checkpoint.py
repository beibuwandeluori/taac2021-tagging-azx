...
from src.data.dataset import create_val_transforms, TencentDatasetInference2
import pickle
...
def inference(model, index_to_tag, eval_loader, top_k=20, output_label=False, output_file_name=""):
    ...
    with torch.no_grad():
        count = 0
        for i, (video_feature, video_feature2, audio_feature, text, image, video_names) in enumerate(eval_loader):
            video_feature = Variable(video_feature.cuda(device_id))
            video_feature2 = Variable(video_feature2.cuda(device_id))
            ...
            if not is_ensemble:
                ...
                y_pred = model[0](video_feature, video_feature2, audio_feature, text, image)
                y_pred = nn.Sigmoid()(y_pred).data.cpu().numpy()
                for model_i in model[1:]:
                    y_pred_i = model_i(video_feature, video_feature2, audio_feature, text, image)
...
if __name__ == '__main__':
    ...
    if args.output_pkl == 1:
        xdl_eval = TencentDatasetInference2(root_path=root_path, class_txt=class_txt, vocab=vocab,
